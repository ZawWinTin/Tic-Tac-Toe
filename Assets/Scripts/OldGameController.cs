...
public class OldGameController : MonoBehaviour
{
    ...
    private ArrayList buttonIds = new ArrayList()
    {
        new int[,]{{1,2},{3,6},{4,8}},      //0 (Winnable buttons)
        new int[,]{{0,2},{4,7}},            //1 (Winnable buttons)
        new int[,]{{0,1},{4,6},{5,8}},      //2 (Winnable buttons)
        new int[,]{{0,6},{4,5}},            //3 (Winnable buttons)
        new int[,]{{0,8},{1,7},{2,6},{3,5}},//4 (Winnable buttons)
        new int[,]{{2,8},{3,4}},            //5 (Winnable buttons)
        new int[,]{{0,3},{2,4},{7,8}},      //6 (Winnable buttons)
        new int[,]{{1,4},{6,8}},            //7 (Winnable buttons)
        new int[,]{{0,4},{2,5},{6,7}}       //8 (Winnable buttons)
    };
    const int middleId = 4;
    private readonly int[] cornerIds = { 0, 2, 6, 8 };
    ...
    public void EndTurn()
    {
        computerFirstButton.SetActive(false);
        moveCount++;
        int[,] checkid = (int[,])buttonIds[lastClickedButtonId];
    ...
    int GetComputerCalculatedMoveId()
    {
        ...
        {
            int[,] attackcheckid = (int[,])buttonIds[i];
        ...
        //Defense
        int[,] defencecheckid = (int[,])buttonIds[lastClickedButtonId];
        ...
        */
        ...
        //Corner
        List<int> computerClickableCornerIds = new List<int>();
        foreach (int cornerId in cornerIds)
        {
            if (buttonList[cornerId].text == "")
            {
                computerClickableCornerIds.Add(cornerId);
            }
        }
        if (computerClickableCornerIds.Count > 0)
        {
            int randomIndex = Random.Range(0, computerClickableCornerIds.Count);
            return computerClickableCornerIds[randomIndex];
        }
